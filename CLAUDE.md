...
AI entrypoint for `@jorgebodega/typeorm-seeding`. Humans start at [README.md](README.md); usage, compatibility, branches and development commands live there and are not repeated here.

Sibling repo: `jorgebodega/typeorm-factory`; this repo uses it in tests.

## Working rules

- Every change is made in this repo **and** in `typeorm-factory` as one task: paired PRs with the same branch and commit message. If one repo needs nothing, it is a no-op, not a separate task.
...
| Path | Purpose |
| --- | --- |
| `src/cli.ts`, `src/commands/seed.command.ts` | `typeorm-seeding seed` CLI (commander + ora) |
| `src/seeder.ts` | abstract `Seeder` |
| `src/helpers/` | `useDataSource`, `useSeeders` |
...
- Shared tooling files must stay byte-identical with `typeorm-factory`: `.commitlintrc`, `.github/workflows/*`, `.github/renovate.json`, `.releaserc.json`, `biome.json`, `jest.config.js`, `pnpm-workspace.yaml`, `tsconfig.json`.
- GitHub Actions are pinned to commit SHAs, with the version as a trailing comment (`uses: owner/action@<sha> # vX.Y.Z`).
- The required status checks on `next` and `main` are the job names `format`, `lint`, `typecheck`, `test (20.19.0)`, `test (22.12.0)`, `test (24.11.0)`, `Test coverage` and `commitlint`. Renaming a job means updating branch protection in both repos, or every PR blocks forever.
...
- Runtime parity with TypeORM: `engines` follows TypeORM's supported Node range; the code compiles with `@tsconfig/node20` (target ES2022, lib ES2023) and `@types/node` 20.x, so no API newer than the lowest supported Node type-checks; output stays CommonJS and ESM consumers load it through Node's CommonJS interop. The `test` job runs on every `engines` floor and checks both `require` and `import` of `dist`. Raise all of these together when the floor changes.
